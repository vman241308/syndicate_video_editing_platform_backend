...
```bash
serverless$ sam build --use-container
```
...
```bash
serverless$ sam logs -n StockCheckerFunction --stack-name serverless --tail
```
...
```bash
serverless$ pip install -r tests/requirements.txt --user
# unit test
serverless$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
serverless$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
